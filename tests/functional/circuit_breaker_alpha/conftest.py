...
class FakePersistence(CircuitBreakerPersistenceLayer):
    ...
    # Each DynamoDB operation is atomic, including the conditional put; the fake must be
    # too, or threaded tests race inside the fake instead of the code under test.
    # Class-level so store instances sharing a db (multi-environment tests) share it.
