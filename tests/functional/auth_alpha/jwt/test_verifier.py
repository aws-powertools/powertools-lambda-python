...
@pytest.mark.parametrize("header", ["Bearer {token}", "bearer {token}", "  BEARER   {token}  "])
def test_verify_authorization_header(jwks, claims, issue_token, issuer_group, header):
    verifier = JWTVerifier(
        issuer="https://idp.example.com/",
        audience="https://api.example.com",
        algorithms=["RS256"],
    ...
    subject = JWTVerifier.any_of(verifier) if issuer_group else verifier

    assert subject.verify_authorization_header(header.format(token=issue_token())) == claims


@pytest.mark.parametrize("header", [None, "", "Basic credential", "Bearer", "Bearer one two", 42])
def test_verify_authorization_header_rejects_missing_or_malformed_values(jwks, header):
    verifier = JWTVerifier(
        issuer="https://idp.example.com/",
        audience="https://api.example.com",
        algorithms=["RS256"],
    ...
    with pytest.raises(InvalidTokenError):
        verifier.verify_authorization_header(header)
