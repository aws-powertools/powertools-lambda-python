...
if scenario == "static":
    ...
    from aws_lambda_powertools.utilities.auth_alpha.jwt.exceptions import InvalidSignatureError
    ...
    auth = importlib.import_module("aws_lambda_powertools.utilities.auth_alpha")
    jwt_auth = importlib.import_module("aws_lambda_powertools.utilities.auth_alpha.jwt")

    exports = {"JWTVerifier", "AuthFailureReason", "AuthErrorContext"}
    assert exports <= set(dir(auth))
    assert exports <= set(dir(jwt_auth))
    ...
    assert members["JWTVerifier"] is jwt_auth.JWTVerifier
    assert members["AuthFailureReason"] is jwt_auth.AuthFailureReason
    assert members["AuthErrorContext"] is jwt_auth.AuthErrorContext
elif scenario == "star":
    from aws_lambda_powertools.utilities.auth_alpha import *  # noqa: E402,F403
