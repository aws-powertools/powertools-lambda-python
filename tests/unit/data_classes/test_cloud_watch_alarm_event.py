...
def test_cloud_watch_alarm_event():
    ...
    assert parsed_event.alarm_data.description == raw_event["alarmData"]["configuration"]["description"]
    assert parsed_event.alarm_data.name == raw_event["alarmData"]["alarmName"]

    assert parsed_event.alarm_data.state.value == "ALARM"
    ...
    assert parsed_event.alarm_data.state.reason_data_decoded["queryDate"] == "2024-02-17T11:53:08.423+0000"
    assert parsed_event.alarm_data.state.timestamp == raw_event["alarmData"]["state"]["timestamp"]

    assert parsed_event.alarm_data.previous_state.value == "OK"
    ...
    assert (
        parsed_event.alarm_data.metrics[0].expression
        == raw_event["alarmData"]["configuration"]["metrics"][0]["expression"]
    )
    assert parsed_event.alarm_data.metrics[0].label == raw_event["alarmData"]["configuration"]["metrics"][0]["label"]
    assert (
        parsed_event.alarm_data.metrics[0].return_data
        == raw_event["alarmData"]["configuration"]["metrics"][0]["returnData"]
    )

    # test the 'metric' metric
    assert parsed_event.alarm_data.metrics[1].metric_id == raw_event["alarmData"]["configuration"]["metrics"][1]["id"]
    assert (
        parsed_event.alarm_data.metrics[1].name
        == raw_event["alarmData"]["configuration"]["metrics"][1]["metricStat"]["metric"]["name"]
    )
    assert (
        parsed_event.alarm_data.metrics[1].namespace
        == raw_event["alarmData"]["configuration"]["metrics"][1]["metricStat"]["metric"]["namespace"]
    )
    assert (
        parsed_event.alarm_data.metrics[1].dimensions
        == raw_event["alarmData"]["configuration"]["metrics"][1]["metricStat"]["metric"]["dimensions"]
    )
    assert (
        parsed_event.alarm_data.metrics[1].return_data
        == raw_event["alarmData"]["configuration"]["metrics"][1]["returnData"]
    )
