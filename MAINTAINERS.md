...
Use and enforce [semantic versioning](https://semver.org/) pull request titles, as these will be used for [CHANGELOG](CHANGELOG.md) and [Release notes](https://github.com/aws-powertools/lambda-python/releases) - make sure they communicate their intent at the human level.
...
Visit the [Releases page](https://github.com/aws-powertools/lambda-python/releases) and choose the edit pencil button.
...
Edit the respective PR title and update their [labels](#labels). Then run the [Release Drafter workflow](https://github.com/aws-powertools/lambda-python/actions/workflows/release-drafter.yml) to update the Draft release.
...
This will kick off the [Publishing workflow](https://github.com/aws-powertools/lambda-python/actions/workflows/release.yml) and within a few minutes you should see the latest version in PyPi, and all issues labeled as `pending-release` will be closed and notified.
...
E2E tests are run on every push to `develop` or manually via [run-e2e-tests workflow](https://github.com/aws-powertools/lambda-python/actions/workflows/run-e2e-tests.yml).
...
You can rebuild the latest documentation without a full release via this [GitHub Actions Workflow](https://github.com/aws-powertools/lambda-python/actions/workflows/rebuild_latest_docs.yml). Choose `Run workflow`, keep `develop` as the branch, and input the latest Powertools for AWS Lambda (Python) version available.
...
We credit all contributions as part of each [release note](https://github.com/aws-powertools/lambda-python/releases) as an automated process. If you find  contributors are missing from the release note you're producing, please add them manually.
...
Under [`BaseInfrastructure`](https://github.com/aws-powertools/lambda-python/blob/develop/tests/e2e/utils/infrastructure.py), we hide the complexity of deployment and delete coordination under `deploy`, `delete`, and `create_lambda_functions` methods.
