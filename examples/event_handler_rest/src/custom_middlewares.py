...
def validate_correlation_id(app: ApiGatewayResolver, get_response: Callable[..., Any], **context_args) -> Response:
...
def sanitise_exceptions(app: ApiGatewayResolver, get_response: Callable[..., Any], **context_args) -> Response:
    try:
        # Get the Result from the next middleware
        result: Response = get_response(app, **context_args)
