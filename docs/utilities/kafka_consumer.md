...
=== "Avro Messages"
    ...
    ```python
    from aws_lambda_powertools.utilities.kafka_consumer import kafka_consumer
...
=== "Protocol Buffers"
    ...
    def lambda_handler(event: ConsumerRecords, context):
        for record in event.records:
            user = record.value  # Dictionary from protobuf message
            print(f"Processing user: {user['name']}, age {user['age']}")
...
=== "JSON Messages"
    ...
    def lambda_handler(event: ConsumerRecords, context):
        for record in event.records:
            user = record.value  # Python dict from JSON
            print(f"Processing user: {user.get('name')}, age {user.get('age')}")
...
=== "Key and Value Deserialization"
    ...
    def lambda_handler(event: ConsumerRecords, context):
        for record in event.records:
            ...
            print(f"Processing product ID: {key['product_id']}")
    ...
    def lambda_handler(event: ConsumerRecords, context):
        for record in event.records:
            ...
            if record.key is not None:
                print(f"Message key: {record.key}")
